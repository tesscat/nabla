...
class SimpleTextOutputter : public TermOutputter {
    std::string content;
public:
    SimpleTextOutputter(std::string string) : content{string} {}
...
std::optional<std::vector<PartialUpdateOutput>> SimpleTextComponent::partialRender(
    const RenderExtent& extent,
    const std::optional<RenderExtent>& oldExtent
) {
    UNUSED(extent);
    UNUSED(oldExtent);
    if (hasBeenSet) return std::optional<std::vector<PartialUpdateOutput>>();
    else return std::optional(std::vector<PartialUpdateOutput>());
}

std::vector<std::shared_ptr<TermOutputter>> SimpleTextComponent::render(const RenderContext& ctx,
                                   const RenderExtent& extent) {
    UNUSED(ctx);
    std::string output;
    ...
    while (currterator < contents.end() && output.size() < extent.height) {
        ...
        size_t thisLineLength = std::min(std::min(nextNewlineDistance, extent.width), remainingLength);
        auto nexterator = currterator;
        utf8::advance(nexterator, thisLineLength, contents.end());
        outputs.push_back(std::string(currterator, nexterator) + std::string(extent.width - thisLineLength, ' '));
        currterator = nexterator;
        if (nextNewlineDistance <= extent.width) currterator++;
    }
    ...
    for (size_t i = 0; i < extent.height; i++) {
        if (i >= outputs.size()) {
            outputters[i].reset(new SimpleTextOutputter(std::string(extent.width, ' ')));
        } else {
